...
public class AdminAuthentication {
	...
	public static Admin authenticateAdmin(String userId, String password) {
		...
		try (SessionFactory factory = configuration.buildSessionFactory()) {
			try (Session session = factory.openSession()) {
				...
				if (isPersonExists(session, userId, password)) {
					// Check if the person with the given ID is an admin
					Admin admin = (Admin) session.createQuery("FROM Admin WHERE person.userId = :userId")
							.setParameter("userId", userId).uniqueResult();

					if (admin != null) {
						return admin; // Person is an admin
					} else {
						System.out.println("Person is not an admin.");
						return null;
					}
				} else {
